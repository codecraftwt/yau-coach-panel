...
// ----------------- TIMESHEET -----------------
  ...
  export const updateTimesheetEntry = async (entryId, updatedData) => {
    try {
      const url = buildApiUrl(API_CONFIG.endpoints.timesheets.update.replace('{id}', entryId));
      return await coachApiCall(url, {
        method: 'PUT',
  ...
  export const deleteTimesheetEntry = async (entryId) => {
    try {
      const url = buildApiUrl(API_CONFIG.endpoints.timesheets.delete.replace('{id}', entryId));
      return await coachApiCall(url, {
        method: 'DELETE'
      });
    } catch (error) {
  ...
  export const getAllLocations = async () => {
    try {
      const url = buildApiUrl(`${API_CONFIG.endpoints.locations.get}`);
      return await coachApiCall(url);
    } catch (error) {
      console.error('Error fetching timesheet entries:', error);
      throw error;
